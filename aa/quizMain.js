...
async function startQuiz(message, quizId, numOfQuizLeft) {
  try {
    if (numOfQuizLeft !== 0) { // If there are questions left continue running the function
      ...
      const fileContents = fs.readFileSync('./questions/' + quiz.quizName.concat('.yml'), 'utf-8');
      ...
      if (yamlFile[numOfQuizLeft].image !== '') {
        quizEmbed.setImage(yamlFile[numOfQuizLeft].image);
      }
    ...
    await message.reply('Error: Invalid quiz ID');
  }
}
